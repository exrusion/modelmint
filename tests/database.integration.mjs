...
const express=(await import('express')).default;const mock=express();mock.use(express.json());let received=null;mock.post('/chat/completions',(req,res)=>{received=req.body;if(req.body.stream){res.set('Content-Type','text/event-stream');res.write('data: '+JSON.stringify({id:'fixture',choices:[{delta:{content:'fixture'}}]})+'\n\n');res.end('data: '+JSON.stringify({choices:[],usage:{prompt_tokens:5,completion_tokens:3}})+'\n\ndata: [DONE]\n\n');}else res.json({id:'fixture',choices:[{message:{role:'assistant',content:'fixture'}}],usage:{prompt_tokens:5,completion_tokens:3}});});upstream=await new Promise(resolve=>{const s=mock.listen(0,'127.0.0.1',()=>resolve(s))});
