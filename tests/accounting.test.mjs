...
import {ceilProduct,usageCost,integer,promoEligible} from '../server/accounting.mjs';
import {encrypt,decrypt} from '../server/db.mjs';
import {verifyInvoice} from '../server/payments.mjs';
import {giftCreditForTokens} from '../server/gifts.mjs';
...
test('playground promo labels match reservation eligibility',()=>{assert.equal(promoEligible({id:'gpt-5.4-mini',name:'GPT-5.4 Mini',promo:true}),true);assert.equal(promoEligible({id:'kimi-k3',name:'Kimi K3',promo:true}),false);assert.equal(promoEligible({id:'gpt-5.6-sol',name:'GPT-5.6 Sol',promo:false}),false);});
test('gifted base tokens carry matching purchased usage credit',()=>{assert.equal(giftCreditForTokens(500000,1000000),500000n);assert.equal(giftCreditForTokens(1,3),333334n);assert.throws(()=>giftCreditForTokens(0,1000000));});
...
test('direct-address EVM invoices accept any sender but still require exact recipient and amount',async()=>{const direct={...order,wallet:'direct'};assert.equal(await verifyInvoice(direct,receipt,{...tx,from:'0xANY'},block,{number:100}),true);await assert.rejects(verifyInvoice(direct,receipt,{...tx,to:'0xBAD'},block,{number:100}));await assert.rejects(verifyInvoice(direct,receipt,{...tx,value:1235n},block,{number:100}));});
