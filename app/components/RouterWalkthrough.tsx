...
function SignupFeed(){
  ...
  const cards=(copy:number)=>signups.map(signup=><a href={`https://x.com/${signup.username}`} target="_blank" rel="noreferrer" tabIndex={copy? -1:0} key={`${copy}-${signup.username}-${signup.created_at}`}><b><span>{signup.username.slice(0,1).toUpperCase()}</span><img src={signup.avatar_url||`https://unavatar.io/x/${encodeURIComponent(signup.username)}`} alt={copy?'':`@${signup.username} profile`} loading="lazy" referrerPolicy="no-referrer" onError={event=>{event.currentTarget.hidden=true}}/></b><span><strong>@{signup.username}</strong><small>Joined {new Date(signup.created_at).toLocaleString(undefined,{month:'short',day:'numeric',hour:'numeric',minute:'2-digit'})}</small></span><em>↗</em></a>);
  return <section className={styles.signupFeed} aria-labelledby="community-signups-title"><div className={styles.signupHeading}><div><span>ROUTERS COMMUNITY</span><h2 id="community-signups-title">Community signups</h2></div><p><i/> {signups.length} LIVE</p></div>{signups.length?<div className={styles.signupGrid}><div className={styles.signupTrack} style={{animationDuration:`${Math.max(30,signups.length*3.5)}s`}}><div className={styles.signupGroup}>{cards(0)}</div><div className={styles.signupGroup} aria-hidden="true">{cards(1)}</div></div></div>:<div className={styles.signupEmpty}>New Router builders will appear here.</div>}<p className={styles.signupPrivacy}>Every public X signup appears here. Wallets, balances and API activity stay private.</p></section>;
}
